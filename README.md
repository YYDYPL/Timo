...
- 新题默认当天进入复习队列；也可勾选「暂不安排复习」，题目会保持待安排状态，随时可在题库点「背」开始。
- 题库每题的「背」按钮可提前复习任意一题，不受今日队列限制。
- 评分支持 `重来(1)`、`困难(3)`、`良好(4)`、`简单(5)`。
- 成功回忆按 `1 天 -> 6 天 -> 上次间隔 × 熟练度` 推进。
- `重来` 会重置连续记忆次数、仍归今天待复习（刷新后依旧在今日队列），并在当前复习会话中再次出现。
- 删除题目时会级联删除它的复习状态和历史记录。

## AI 能力

- 项目追问生成时会同时产出参考答案与关键要点。
- 题库编辑弹窗里的「用 AI 生成参考答案」可为空答案题目一键补全；「用 AI 生成要点」可依据题目与参考答案提炼 3-7 条关键要点（均需配置 LLM）。
- 时间戳与复习排期均使用本地时间，避免跨时区日期错位。
